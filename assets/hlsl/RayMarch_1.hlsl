...
float sdfScene(float3 samplePoint)
{
    ...
    float sphereDist = sdfSphere(samplePoint, float3(PosX, PosY, PosZ), 1.0f);
...
float Cloud(float3 rayDir, float3 viewPos, float3 viewDir, float3 lightDir, float near, float far, float4x4 world)
{
    ...
    for (int i = 0; i < MAX_STEPS; i++)
    {
        ...
        float3 texSample = samplePoint;
        float3 normal = ApproximateNormal(samplePoint);
		
        float cloudSample = mCloudTexture.Sample(DefaultSamplerState, samplePoint).x;
        float sceneDist = sdfScene(samplePoint);
        
        if (sceneDist < EPSILON) // within unit size
        {
    ...
    return 1.0f / exp(attenuation * depth);
